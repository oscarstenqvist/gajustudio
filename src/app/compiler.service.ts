...
export class CompilerService {
  ...
  public onWalletSearchSuccess = async (connection, walletInfo: WalletInfo) => {
    ...
    const {
      address: { current },
    } = await (this.Chain as AeSdkAeppExtended).subscribeAddress(
  ...
  constructor(
    ...
    this.justScanForWallets(() => {
      this.walletExtensionPresent = true;
    });
  }

  // is ran, when the browser addon wallet is not to be used ("testnet")
  async setupWebClient(_config?: {
    nodeUrl?: string;
    compilerUrl?: string;
    personalAccounts?: boolean;
    accounts?: (typeof MemoryAccount)[];
    command?: string;
  }) {
    ...
    if (this.Chain != undefined) {
      ...
      this.SDKoptionsToCheck.forEach((setting) => {
        if (setting == 'addresses') {
          ...
          //when browser wallet:
          returnObject.address = Object.keys(
            (this.Chain as AeSdkAeppExtended)._accounts.current,
          )[0];
        }
      });
      return returnObject;
  ...
  async compileAndDeploy(
    _deploymentParams,
    _existingContractAddress?: `ct_${string}` | `${string}.chain`,
  ): Promise<any> {
    ...
    if (!_existingContractAddress) {
      ...
      try {
        ...
        this.logMessage({
          type: 'success',
          message: 'Contract successfully deployed: ' + myContract._name,
          data: myContract.deployInfo,
        });
        ...
        if (typeof _e === 'object') {
          this.logMessage({
            type: 'error',
            message: 'Contract deployment failed: ' + myContract._name,
      ...
      this.logMessage({
        type: 'success',
        message: 'Successfully casted contract at: ' + myContract._name,
        data: {},
      });
    }
  ...
  private defaultOrCustomSDKsetting(_setting) {
    ...
    return this.sdkConfigOverrides[_setting] == undefined
      ? this.defaultSdkConfig[_setting]
      : this.sdkConfigOverrides[_setting];
  }
